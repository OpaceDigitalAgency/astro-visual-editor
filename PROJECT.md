...
| GitHub protection settings    | Pending repository-admin setup | Checked on 5 August 2026: no branch protection rule is configured yet; `main` protection, the `npm-publish` approval environment and a `v*` tag ruleset remain required before relying on the guarded workflow                                           |
