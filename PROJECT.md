...
Version `0.1.0-beta.4` of `@opacedev/astro-visual-editor` is published on npm
after owner acceptance and protected publication. It adds the guided
Editability Setup and page inventory to Beta 3's durable recovery, exact review
and compact editor.
...
| Source commit/revert          | Complete                              | Exact pre-write source review, real commit, HMR/restart receipt recovery and conflict-protected restore pass in beta.4 tests                                                                                                                                                                                                |
...
| GitHub Actions run            | Complete                              | Node 22.12 and 24 passed on the exact beta.4 `main` commit `ce645bd` ([run #31021104027](https://github.com/OpaceDigitalAgency/astro-visual-editor/actions/runs/31021104027))                                                                                                                                               |
...
| Release automation            | Complete                              | The protected workflow created `v0.1.0-beta.4`, published through npm OIDC with provenance and verified the registry package ([run #31021372372](https://github.com/OpaceDigitalAgency/astro-visual-editor/actions/runs/31021372372))                                                                                       |
| Future release guardrails     | Complete in repository                | Candidate CI completes before owner testing; a dependency-free version/document/fixture check fails early; approval finalisation safely restores demo writes and freezes the pushed candidate; protected publication records an automatic workflow summary; required settings are in `RELEASE_GUARDRAILS.md`                |
...
| npm publication               | Complete                              | [`@opacedev/astro-visual-editor@0.1.0-beta.4`](https://www.npmjs.com/package/@opacedev/astro-visual-editor/v/0.1.0-beta.4) is public; `beta` resolves to beta.4 and `latest` intentionally remains beta.2                                                                                                                   |
| Real registry consumer        | Complete                              | The protected release workflow installed exact beta.4 into a fresh Astro 7.1.6 project and completed a production build                                                                                                                                                                                                     |
| Astro importer eligibility    | Complete; search reindex pending      | Required npm metadata remains present, the package is absent from Astro's blocklist and Astro's unmodified importer previously generated the expected entry and four category mappings; npm search reindexing for beta.4 is externally pending                                                                              |
